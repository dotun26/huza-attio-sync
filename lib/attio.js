...
// Get all people from Attio (via search/workspace API)
export async function getAllPeople(limit = 100) {
    const people = [];
    let page = 0;
    let hasMore = true;
    ...
    while (hasMore) {
        try {
            console.log(`[attio] Fetching people page ${page}...`);
            
            // Use workspace records endpoint with pagination
            const res = await fetch(
                `${ATTIO_BASE}/workspaces/7e3caac8-6e73-4b9c-8b03-a1a19988f15a/people?limit=${limit}&offset=${page * limit}`,
                {
                    method: 'GET',
            ...
            if (!res.ok) {
                // Fallback: try simple search if workspace endpoint fails
                console.log(`[attio] Workspace endpoint failed (${res.status}), trying search...`);
                const searchRes = await fetch(
                    `${ATTIO_BASE}/search?q=*&type=people&limit=${limit}&offset=${page * limit}`,
                    { headers: ATTIO_HEADERS }
                );
                ...
                const batch = data.records || data.data || [];
                people.push(...batch);
